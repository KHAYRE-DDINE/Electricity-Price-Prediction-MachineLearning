...
## 9. تفاصيل المنهجية (Methodology Details)

بناءً على الكود والوثائق في مشروعك، فإن طريقة تعلم الآلة المستخدمة هي **الانحدار الخطي (Linear Regression)**.

فيما يلي التفاصيل المحددة للتنفيذ:

### 1. الخوارزمية (The Algorithm)

- **النموذج:** `LinearRegression` من مكتبة **scikit-learn**.
- **النوع:** خوارزمية انحدار ضمن **التعلم الخاضع للإشراف (Supervised Learning)**.
- **الهدف:** تقوم بملاءمة معادلة خطية مع البيانات المرصودة للتنبؤ بالقيمة الرقمية لسعر الكهرباء.

### 2. لماذا هذه الطريقة؟ (Why this method?)

- **خط أساس (Baseline):** غالباً ما يستخدم الانحدار الخطي كنموذج أولي لإنشاء معيار للأداء يمكن مقارنته بالنماذج الأخرى لاحقاً.
- **قابلية التفسير (Interpretability):** من السهل فهم كيفية تأثير كل ميزة (مثل درجة الحرارة أو الطلب) على السعر النهائي (ارتباط إيجابي أو سلبي).
- **السرعة (Speed):** تدريب النموذج سريع جداً من الناحية الحسابية مقارنة بالنماذج المعقدة مثل الشبكات العصبية.

### 3. التقنيات الرئيسية المستخدمة مع النموذج (Key Techniques)

لجعل الانحدار الخطي يعمل بشكل أفضل، يطبق الكود تقنيات المعالجة المسبقة التالية:

- **توحيد المقياس (Feature Scaling):** يستخدم `StandardScaler` لتطبيع البيانات (جعل المتوسط = 0 والتباين = 1). هذا أمر بالغ الأهمية للانحدار الخطي لمنع الميزات ذات الأرقام الكبيرة (مثل الطلب ~ 5000) من الهيمنة على الميزات ذات الأرقام الصغيرة (مثل درجة الحرارة ~ 25).
- **إزالة القيم الشاذة (Outlier Removal):** يستخدم طريقة **المدى الربيعي (IQR)** لإزالة نقاط البيانات المتطرفة التي قد تؤدي إلى انحراف خط الانحدار.

### 4. المدخلات والمخرجات (Input & Output)

- **ميزات الإدخال ($X$):**
  - درجة الحرارة في الأوقات $t$، $t-1$، $t-2$.
  - الطلب (الحمل) في الأوقات $t$، $t-1$، $t-2$.
- **المخرجات المستهدفة ($y$):**
  - سعر الكهرباء في الوقت $t+1$.
